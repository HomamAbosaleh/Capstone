...
class RobotController:
    def __init__(self):
        ...
        cap = cv2.VideoCapture("nvarguscamerasrc sensor-id=%d ! "
    ...
    def calculate_bearning_and_distance(img_w, x1, x2, y1, y2):
        # Coefficients
        a = 394.1
        b = -0.000153
        c = 242.1
        ...
        return atan_x, a * np.exp(b * (x2 - x1) * (y2 - y1)) + c * np.exp(d * (x2 - x1) * (y2 - y1))
